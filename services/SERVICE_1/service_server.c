...
void send_to_server(int server_socket, char packet[]) {
    if (packet != 0) {
        ...
        printf("PYKE SERVICE 1 Message sended to main server : %s\n", packet);
    }
}
...
int connect_client(void) {
    ...
    printf("\nPYKE SERVICE 1 SOCKET C - By LA\nConnecting to socket server...\n");

    // Create new socket
    if ((server_socket = socket(AF_UNIX, SOCK_STREAM, 0)) == 0) {
        perror(" PYKE SERVICE 1 Error : Socket creation");
        exit(EXIT_FAILURE);
    }
    ...
    if (connect(server_socket, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) { // Connect to server
        perror("PYKE SERVICE 1 Error : Connecting to server");
        exit(EXIT_FAILURE);
    }
