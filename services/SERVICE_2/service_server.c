...
char* receive_from_server(int server_socket) {
    ...
    printf("PYKE SERVICE 2 Message received from main server : %s\n", message);
    return message;
}
...
void send_to_server(int server_socket, char packet[]) {
    if (packet != 0) {
        ...
        printf("PYKE SERVICE 2 Message sended to main server : %s\n", packet);
    }
}
...
int connect_client(void) {
    ...
    printf("\nPYKE SERVICE 2 SOCKET C - By LA\nConnecting to socket server...\n");

    // Create new socket
    if ((server_socket = socket(AF_UNIX, SOCK_STREAM, 0)) == 0) {
        perror("PYKE SERVICE 2 Error : Socket creation");
        exit(EXIT_FAILURE);
    }
    ...
    if (connect(server_socket, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) { // Connect to server
        perror("PYKE SERVICE 2 Error : Connecting to server");
        exit(EXIT_FAILURE);
    }

    send_to_server(server_socket, "1");
